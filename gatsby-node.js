...
function createBlogPages({ boundActionCreators, graphql }) {
  ...
  return graphql(blogPostQuery).then(result => {
    if (result.errors) {
...
function createArchivePages({ boundActionCreators, graphql }) {
  ...
  return graphql(blogPostQuery).then(result => {
    ...
    const articlesPerMonth = result.data.allMarkdownRemark.edges.reduce(
      (months, { node }) => {
        const previousArticlesPerMonth = months[node.frontmatter.date]
        months[node.frontmatter.date] = previousArticlesPerMonth
          ? [...previousArticlesPerMonth, node]
          : [node]
        return months
      },
      {}
    ...
    Object.keys(articlesPerMonth).forEach(key =>
      articlesPerMonth[key].forEach(node => {
        createPage({
